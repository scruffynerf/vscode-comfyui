...
Five bugs that shared a failure mode: the extension reported success while nothing happened. Three of them wrote `ok` to `apply-response.json` for operations the canvas never performed.

- **`patchPath` and `sourcePath` triggers were unreachable code**: in `patchBridge.ts` the `restart-server` branch's `catch` block is never closed — its `}` closes the `if (!isExpectedDrop)` guard instead. Everything after it (`refresh-catalog`, `open-panel`, `analyze-workflow`, and the entire patch/workflow apply block) ended up nested inside `if (command === 'restart-server')`. Those triggers never ran, and because they fell through without reaching any `writeApplyResponse` call, **nothing was written to `apply-response.json` at all** — no result, no error. Commands that return earlier (`queue`, `interrupt`, `auto-layout`, `queue-status`, `testing-mode`, `wiki-mode`) were unaffected, which is why the bridge appeared half-functional rather than broken. An unrecognised trigger now reports an error instead of vanishing.

- **Patches could not reach nodes inside subgraphs**: since the subgraph rework, node IDs live in per-graph ID spaces. The bridge node resolved every patch ID with `getNodeById()` on the root graph only, so a patch aimed at a node inside a subgraph found nothing, skipped it, and reported success. On a workflow with two subgraphs that was 87 of 102 nodes — every steps, CFG, width, height and seed control. Resolution now walks `[rootGraph, ...rootGraph.subgraphs.values()]`. Because IDs are unique only *within* a graph, a patch node may now carry `"subgraph": "<name|uuid|root>"` to pin the scope; an ID that matches in more than one graph is reported as ambiguous rather than guessed at.

- **`apply-response.json` described the request, not the result**: it was generated by counting entries in the patch file, so a patch naming an unknown node and one that applied cleanly produced identical `ok` messages. It also reported success when the canvas never received the patch — the code only checked that `ComfyUIPanel.currentPanel` exists, but the panel object outlives a webview reload and `postMessage` into a webview whose iframe is gone succeeds silently. After an `open-panel` reload the panel looked healthy while discarding every patch. `applyPatch()` now returns a promise resolved by an `applyPatchResult` acknowledgement correlated by `requestId`, and the response is built from the bridge's own per-node report:
  - `ok — Patch applied: 1 change(s) — node 168 [SCAIL-2 Base] widgets_values[0..1]`
  - `error — Patch applied: 2 change(s) — … 1 problem(s): node 9999: not found …`
  - `error — Patch NOT applied — …bridge did not acknowledge… reload the panel`

- **Panel reload did not verify the bridge came back**: `reload()` blanked the webview HTML, restored it 100 ms later and assumed success. It is now async, waits for a `bridgeReady` heartbeat, retries once with a longer settle, and returns a boolean that `open-panel` and `restart-server` actually use. `queue` also checks bridge liveness first, so it can no longer claim to have queued work into a dead canvas.

- **Server restart never worked (#6)**: `comfyui.restartServer` and the `restart-server` trigger both issued `GET /v2/manager/reboot`, but ComfyUI-Manager declares `@routes.post(...)` and registers no GET handler, so the request always 404'd. Verified against ComfyUI-Manager V4.2.2: `GET` returns 404, `POST` reboots. A 404 or 405 now hints that the server may not have been started with `--enable-manager`, which is the more common cause than a wrong endpoint.

- **Node catalog build crashed on integer combo options**: `isModelFileList()` was typed `string[]` and called `toLowerCase()` on every combo option, but real `/object_info` payloads include integers — `SaveImagesResponse.bits` is `[8, 16]`, `WanVideoSetRadialAttention.block_size` is `[128, 64]`, `ImageResizeKJ.crop` contains `0`. The throw aborted the whole catalog build partway, leaving some index files rewritten and `widget-enums.json` never written. This was unreachable until the brace fix above restored `refresh-catalog`, which is why it had gone unnoticed.

### Bridge node fixes

- **Links could not be created**: `connect()` takes a node *object* in the current ComfyUI frontend, but the bridge passed a numeric node ID. Link patches were broken independently of the subgraph issue. Endpoints that live in different graphs are now refused with an explicit message rather than failing obscurely.
- **New nodes could collide with hand-added ones**: creating a node no longer leaves the target graph's `last_node_id` behind, so the next node added in the UI cannot reuse a patched-in ID.
- **Patch results are now readable from the canvas side**: the bridge attaches its per-node report to the state broadcast as `_vscode_patch` in `workflow-state.readonly.json`, carrying `{ts, applied: [{id, graph, changed}], errors: []}`.

### Notes

- Patch-supplied link IDs are ignored — LiteGraph assigns its own. Read the real ID back from `workflow-state.readonly.json` before referencing it in `remove_links`.
- Root-graph links serialize as arrays (`[id, origin, origin_slot, target, target_slot, type]`) while subgraph links serialize as objects (`{id, origin_id, origin_slot, target_id, target_slot, type}`). Handle both when parsing workflow state.
- Widget values are indexed by widget array position, matching what `serialize()` writes, so a patch expressed against `workflow-state.readonly.json` round-trips. Note that litegraph's own `configure()` reads them back compacted, skipping `serialize === false` widgets — the two disagree if such a widget is present.
